...
struct Point {
  ...
  Point(const uint64_t position, const float value) : position(position), value(value) {
  }
...
/** A simple vector-of-vectors adaptor for nanoflann, without duplicating the storage.
  ...
  */
template <typename num_t = float, int DIM = -1, class Distance = nanoflann::metric_L2, typename IndexType = size_t>
struct SigmapAdaptor {
  ...
  SigmapAdaptor(const size_t dims/* dimensionality */, const std::vector<Point> &mat, const int leaf_max_size = 10) : m_data(mat) {
