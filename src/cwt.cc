...
cwt_type factorial(int N) {
  //static const cwt_type fact[41] = { 1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600, 6227020800, 87178291200, 1307674368000,
  ...
  static const cwt_type fact[14] = {1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600, 6227020800};
//  if (N > 40 || N < 0) {
...
cwt_type cwt_gamma(cwt_type x) {
  ...
  cwt_type num[8] = { -1.71618513886549492533811e+0,
  ...
  cwt_type den[8] = { -3.08402300119738975254353e+1,
...
static void wave_function(int nk, cwt_type dt,int mother, cwt_type param, cwt_type scale1, cwt_type *kwave, cwt_type pi, cwt_type *period1, cwt_type *coi1, fft_data *daughter) {
  ...
  if (mother == 0) {
    ...
    *coi1 = fourier_factor / sqrt(2.0);
    ...
    m = (int)param;
    norm = sqrt(2.0 * pi * scale1 / dt) * (pow(2.0, (cwt_type)m) / sqrt((cwt_type)(m * factorial(2 * m - 1))));
    for (k = 1; k <= nk / 2 + 1; ++k) {
    ...
    *coi1 = fourier_factor * sqrt(2.0);
    ...
    if (re == 1) {
      ...
      for (k = 1; k <= nk; ++k) {
        temp = scale1 * kwave[k - 1];
        daughter[k - 1].re = 0.0;
        daughter[k - 1].im = norm * pow(temp, (cwt_type)m) * exp(-0.50 * pow(temp, 2.0));
      }
    }
...
cwt_object cwt_init(const char* wave, cwt_type param, int siglength, cwt_type dt, int J) {
  ...
  N = siglength;
  nj2 = 2 * N * J;
  obj = (cwt_object)malloc(sizeof(struct cwt_set) + sizeof(cwt_type) * (nj2 + 2 * J + N));
